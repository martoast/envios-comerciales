<!-- pages/app/admin/customers/index.vue -->
<template>
  <section
  ...
  >
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      >
        ...
        >
          <div
            class="inline-flex items-center justify-center w-20 h-20 bg-gray-100 rounded-2xl mb-4"
          >
            <svg
            ...
            >
              <div class="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p v-if="customer.phone" class="text-xs text-gray-500 mt-1">
                    {{ customer.phone }}
                  </p>
                </div>
...
const stats = computed(() => {
  const customersForStats =
    allCustomers.value.length > 0 ? allCustomers.value : customers.value;
  const totalCustomers = customersForStats.length;
  const withActiveOrders = customersForStats.filter(
    (c) => c.active_orders_count > 0
  ).length;
  ...
  const newThisMonth = customersForStats.filter(
    (c) => new Date(c.created_at) >= thisMonth
  ).length;

  // Calculate average orders per customer
  const totalOrders = customersForStats.reduce(
