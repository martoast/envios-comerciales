<template>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
              ...
              <div v-if="customerData">
                <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900">{{ customerData.customer.name }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ t.customerSince }} {{ customerData.stats.member_since }}</p>
              </div>
              <div v-else>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div v-else-if="customerData" class="space-y-6">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn" style="animation-delay: 0.4s">
              ...
              <div class="space-y-3">
                ...
                <div v-if="customerData.customer.provider">
                  ...
                  <p class="font-medium text-gray-900 capitalize">{{ customerData.customer.provider }}</p>
                </div>
                <div v-if="customerData.customer.email_verified_at">
                  <p class="text-sm text-gray-500">{{ t.emailVerified }}</p>
                  <p class="font-medium text-gray-900">{{ formatDate(customerData.customer.email_verified_at) }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn" style="animation-delay: 0.5s">
              <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                ...
                {{ t.recentDeliveryAddress }}
              </h2>
              <div v-if="mostRecentAddress" class="space-y-1 text-sm text-gray-900">
                <p>{{ mostRecentAddress.street }} {{ mostRecentAddress.exterior_number }}</p>
                <p v-if="mostRecentAddress.interior_number">Int. {{ mostRecentAddress.interior_number }}</p>
                <p>{{ mostRecentAddress.colonia }}</p>
                <p>{{ mostRecentAddress.municipio }}, {{ mostRecentAddress.estado }}</p>
                <p>C.P. {{ mostRecentAddress.postal_code }}</p>
                <p v-if="mostRecentAddress.referencias" class="text-gray-500 mt-2">
                  <span class="font-medium">{{ t.references }}:</span> {{ mostRecentAddress.referencias }}
                </p>
              </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn" style="animation-delay: 0.6s">
              ...
              <div v-if="customerData.customer.stripe_id" class="space-y-3">
                <div>
                  ...
                  <p class="font-medium text-gray-900 text-xs">{{ customerData.customer.stripe_id }}</p>
                </div>
                <div v-if="customerData.customer.pm_type && customerData.customer.pm_last_four">
                  <p class="text-sm text-gray-500">{{ t.paymentMethod }}</p>
                  <p class="font-medium text-gray-900">
                    <span class="capitalize">{{ customerData.customer.pm_type }}</span> •••• {{ customerData.customer.pm_last_four }}
                  </p>
                </div>
              </div>
              <div v-else class="text-sm text-gray-500">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fadeIn" style="animation-delay: 0.7s">
            ...
            <div v-if="customerData.customer.orders && customerData.customer.orders.length > 0">
              <!-- Desktop Table -->
                    ...
                    <tr v-for="order in customerData.customer.orders" :key="order.id" class="hover:bg-gray-50 transition-colors">
                      ...
                      <td class="px-6 py-4 text-sm text-gray-900">
                        <div v-if="order.delivery_address">
                          <p class="text-xs">{{ order.delivery_address.municipio }}, {{ order.delivery_address.estado }}</p>
                          <span v-if="order.is_rural" class="text-xs text-yellow-600">{{ t.rural }}</span>
                        </div>
                        <span v-else class="text-gray-500">-</span>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <span v-if="order.amount_paid">${{ formatCurrency(order.amount_paid) }} MXN</span>
                        <span v-else class="text-gray-500">-</span>
                      </td>
                ...
                >
                  ...
                  <div class="text-sm space-y-1">
                    <div v-if="order.delivery_address" class="text-gray-600">
                      <span class="font-medium">{{ t.address }}:</span> {{ order.delivery_address.municipio }}, {{ order.delivery_address.estado }}
                    </div>
                    <div class="flex items-center justify-between">
          ...
          <!-- Customer Timeline -->
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn" style="animation-delay: 0.8s">
            ...
            <div class="space-y-4">
              <div v-for="(event, index) in customerTimeline" :key="index" class="flex gap-4">
                <div class="flex flex-col items-center">
                  <div :class="[
                    'w-4 h-4 rounded-full',
                    event.completed ? 'bg-primary-500' : 'bg-gray-300'
                  ]"></div>
                  <div v-if="index < customerTimeline.length - 1" class="w-0.5 h-16 bg-gray-200"></div>
                </div>
                <div class="flex-1 pb-8">
                  <p class="font-medium text-gray-900">{{ event.label }}</p>
                  <p v-if="event.date" class="text-sm text-gray-500">{{ event.date }}</p>
  ...
  const translations = {
    ...
    recentDeliveryAddress: {
      es: 'Dirección de Entrega Reciente',
      en: 'Recent Delivery Address'
    },
    deliveryAddress: {
    ...
    noAddress: {
      es: 'No hay direcciones en las órdenes',
      en: 'No addresses in orders'
    },
    paymentInfo: {
  ...
  // Computed
  const mostRecentAddress = computed(() => {
    if (!customerData.value?.customer?.orders || customerData.value.customer.orders.length === 0) {
      return null
    }
    
    // Find the most recent order with a delivery address
    const orderWithAddress = customerData.value.customer.orders.find(order => order.delivery_address)
    return orderWithAddress?.delivery_address || null
  })
    ...
    return [
      {
        ...
        value: `$${formatCurrency(customerData.value.stats.total_spent || 0)} MXN`,
      ...
      },
      {
        ...
        value: `$${formatCurrency(customerData.value.stats.average_order_value || 0)} MXN`,
    ...
    const events = [
      {
        ...
        description: customerData.value.customer.provider ? `Via ${customerData.value.customer.provider}` : null,
        completed: true
      }
    ]
