...
const useSimpleAddress = ref(true) // Default to simple mode for admins
...
const translations = {
  ...
  createOrderSubtitle: { es: 'Selecciona el cliente y configura la dirección de entrega', en: 'Select customer and configure delivery address' },
...
const selectUser = (user) => {
  ...
  if (user.street) {
    if (useSimpleAddress.value) {
      // Build full address string from user's saved address
      const parts = [
        user.street,
...
const handleSubmit = async () => {
  ...
  try {
    // Build delivery_address based on mode
    let deliveryAddress
    
    if (useSimpleAddress.value) {
      // Simple mode: just send full_address
      deliveryAddress = {
        full_address: form.value.delivery_address.full_address
      }
    } else {
      // Detailed mode: send individual fields (without full_address)
      deliveryAddress = {
