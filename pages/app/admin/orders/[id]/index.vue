<template>
    <section class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-primary-50/20">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
              ...
              <div>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900">{{ order?.order_name || t.loading }}</h1>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div v-else-if="order" class="space-y-6">
          <!-- Quick Actions -->
          <div v-if="showQuickActions" class="bg-amber-50 border border-amber-200 rounded-2xl p-4 sm:p-6 animate-fadeIn">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <h3 class="text-lg font-semibold text-amber-900">{{ t.actionRequired }}</h3>
                <p class="text-sm text-amber-700 mt-1">{{ getActionMessage() }}</p>
                <button 
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn" style="animation-delay: 0.1s">
              ...
              <div class="space-y-3">
                ...
                <div v-if="order.recommended_box_size">
                  ...
                  <p class="font-medium text-gray-900 capitalize">{{ order.recommended_box_size }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn" style="animation-delay: 0.2s">
              ...
              <div class="space-y-3">
                <div v-if="order.delivery_address">
                  <p class="text-sm text-gray-500 mb-1">{{ t.deliveryAddress }}</p>
                  <div class="text-sm text-gray-900 space-y-1">
                    <p>{{ order.delivery_address.street }} {{ order.delivery_address.exterior_number }}</p>
                    <p v-if="order.delivery_address.interior_number">Int. {{ order.delivery_address.interior_number }}</p>
                    <p>{{ order.delivery_address.colonia }}</p>
                    <p>{{ order.delivery_address.municipio }}, {{ order.delivery_address.estado }}</p>
                    <p>C.P. {{ order.delivery_address.postal_code }}</p>
                  </div>
                  <span v-if="order.is_rural" class="inline-flex items-center gap-1 px-2 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full mt-2">
          ...
          <div v-if="order.status !== 'collecting' && order.status !== 'awaiting_packages' && order.status !== 'packages_complete'" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div class="bg-gray-50 rounded-lg p-4">
                ...
                <p class="text-xl font-bold text-gray-900">${{ formatCurrency(order.amount_paid || 0) }} MXN</p>
              </div>
              <div class="bg-gray-50 rounded-lg p-4">
                <p class="text-sm text-gray-500">{{ t.status }}</p>
                <p class="text-xl font-bold text-gray-900">{{ order.amount_paid ? t.paid : t.pending }}</p>
              </div>
              <div v-if="order.stripe_invoice_url" class="bg-gray-50 rounded-lg p-4">
                <p class="text-sm text-gray-500">{{ t.invoice }}</p>
                <a :href="order.stripe_invoice_url" target="_blank" class="text-primary-600 hover:text-primary-700 font-medium inline-flex items-center gap-1">
                  {{ t.viewInvoice }}
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-100">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t.tracking }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                  <tr v-for="item in order.items" :key="item.id" class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4">
                      <div>
                        <a v-if="item.product_url" :href="item.product_url" target="_blank" class="text-primary-600 hover:text-primary-700">
                          {{ t.viewProduct }}
                        </a>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <div>
                        <p class="text-sm text-gray-900">{{ item.tracking_number || '-' }}</p>
                        <p v-if="item.carrier" class="text-xs text-gray-500">{{ item.carrier }}</p>
                      </div>
                    </td>
                    ...
                    <td class="px-6 py-4">
                      <span :class="[
                        ...
                        item.arrived ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                      ]">
              ...
              <div v-for="item in order.items" :key="item.id" class="p-4">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p class="font-medium">${{ item.declared_value }} × {{ item.quantity }}</p>
                  </div>
                  <div>
                    <p class="text-gray-500">{{ t.weight }}</p>
                    <p class="font-medium">{{ item.weight ? `${item.weight} kg` : '-' }}</p>
                  </div>
                </div>
                <button 
                  v-if="!item.arrived"
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn">
            ...
            <div class="space-y-4">
              <div v-for="(event, index) in orderTimeline" :key="index" class="flex gap-4">
                <div class="flex flex-col items-center">
                  <div :class="[
                    'w-4 h-4 rounded-full',
                    event.completed ? 'bg-primary-500' : 'bg-gray-300'
                  ]"></div>
                  <div v-if="index < orderTimeline.length - 1" class="w-0.5 h-16 bg-gray-200"></div>
                </div>
                <div class="flex-1 pb-8">
                  <p class="font-medium text-gray-900">{{ event.label }}</p>
                  <p v-if="event.date" class="text-sm text-gray-500">{{ formatDate(event.date) }}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              >
                <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-xl transition-all">
                  <div class="p-6">
                    ...
                    <div class="space-y-4">
                      <div class="bg-gray-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600 mb-3">{{ t.quoteDetails }}</p>
                        <div class="space-y-2">
                          <div class="flex justify-between text-sm">
                            <span class="text-gray-600">{{ t.boxSize }}:</span>
                            <span class="font-medium capitalize">{{ order.recommended_box_size || calculateBoxSize() }}</span>
                          </div>
                          <div class="flex justify-between text-sm">
                            <span class="text-gray-600">{{ t.totalWeight }}:</span>
                            <span class="font-medium">{{ order.total_weight || calculateTotalWeight() }} kg</span>
                          </div>
                          <div class="flex justify-between text-sm">
                            <span class="text-gray-600">{{ t.shippingCost }}:</span>
                            <span class="font-medium">${{ formatCurrency(calculateShippingCost()) }} MXN</span>
                          </div>
                          <div class="flex justify-between text-sm">
                            <span class="text-gray-600">{{ t.iva }}:</span>
                            <span class="font-medium">${{ formatCurrency(calculateIva()) }} MXN</span>
                          </div>
                          <div class="border-t pt-2 flex justify-between">
                            <span class="font-medium">{{ t.total }}:</span>
                            <span class="font-bold text-lg">${{ formatCurrency(calculateTotal()) }} MXN</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white shadow-xl transition-all">
                  <div class="p-6">
                    ...
                    <div class="mt-6 flex gap-3">
                      <button
                        @click="markAsShipped"
                        :disabled="!shipForm.tracking_number || !shipForm.estimated_delivery_date || updatingStatus"
                        class="flex-1 px-4 py-2 bg-primary-500 text-white font-medium rounded-lg hover:bg-primary-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
  ...
  const orderTimeline = computed(() => {
    ...
    const events = [
      ...
      },
      {
        ...
        completed: ['packages_complete', 'quote_sent', 'paid', 'shipped', 'delivered'].includes(order.value.status)
      },
      {
  ...
  const sendQuote = async () => {
    ...
    try {
      const response = await $customFetch(`/admin/orders/${order.value.id}/send-quote`, {
        method: 'POST'
      })
