<template>
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <!-- <div>
                  <p class="text-sm text-gray-500">{{ t.totalDeclaredValue }}</p>
                  <p class="font-medium">${{ calculateTotalDeclaredValue() }} USD</p>
                </div> -->
              </div>
            </div>
            ...
            <div v-if="!order.quote_breakdown || order.status === 'packages_complete'" class="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div class="space-y-4">
                <!-- Box Size Selection from Products API -->
                <div>
                  ...
                  >
                    <option value="">{{ t.selectBoxOption }}</option>
                    <option
                      v-for="box in availableBoxes"
                      :key="box.id"
                      :value="box.id"
                    >
                      {{ box.name }} - {{ box.dimensions }} (Max: {{ box.max_weight }}kg) - ${{ box.price.toFixed(2) }} MXN
                    </option>
                  </select>
                </div>
  ...
  const fetchBoxProducts = async () => {
    ...
    try {
      ...
      // If order has a box size, select it after boxes are loaded
      if (order.value?.box_size) {
        const matchingBox = availableBoxes.value.find(box => box.type === order.value.box_size)
        if (matchingBox) {
          selectedBoxId.value = matchingBox.id
          selectedBox.value = matchingBox
        }
  ...
  const selectBox = () => {
    ...
    if (box) {
      ...
      quoteForm.value.box_size = box.type
  ...
  const getQuotePreview = () => {
    ...
    // Box charge from selected box (includes shipping to Mexico)
    if (selectedBox.value) {
      items.push({
        item: selectedBox.value.name + ' (incluye envío)',
  ...
  const prepareQuote = async () => {
    ...
    try {
      ...
      if (selectedBox.value) {
        quoteItems.push({
          item: selectedBox.value.name,
          description: `Caja consolidada ${selectedBox.value.dimensions} - Incluye envío a México`,
          amount: selectedBox.value.price,
          type: 'box'
        })
      }
      ...
      const body = {
        ...
        box_size: selectedBox.value.type,
