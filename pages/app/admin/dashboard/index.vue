      ...
      <div v-else class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-border p-6 animate-fadeIn" style="animation-delay: 0.15s">
              ...
              <p class="text-xs font-medium text-text-secondary uppercase tracking-wider mb-1">{{ t.purchaseRequests }}</p>
              <p class="text-2xl font-bold text-text-primary">{{ dashboardData?.financial?.metrics?.purchase_requests_count || 0 }}</p>
              <p class="text-xs text-text-secondary mt-1">{{ dashboardData?.financial?.metrics?.purchased_items_count || 0 }} {{ t.itemsPurchased }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl shadow-sm border border-green-200 p-6 animate-fadeIn" style="animation-delay: 0.4s">
            ...
            <!-- Revenue Breakdown -->
            <div v-if="dashboardData?.financial?.revenue?.breakdown" class="mt-3 pt-3 border-t border-green-200/50 text-xs space-y-1">
               <div class="flex justify-between text-green-800">
                   <span>{{ t.shippingRevenue }}:</span>
                   <span class="font-semibold">${{ formatMoney(dashboardData.financial.revenue.breakdown.shipping || 0) }}</span>
               </div>
               <div class="flex justify-between text-green-800">
                   <span>{{ t.serviceFeeRevenue }}:</span>
                   <span class="font-semibold">${{ formatMoney(dashboardData.financial.revenue.breakdown.service_fees || 0) }}</span>
               </div>
            </div>
            
            <p v-else class="text-xs text-green-700 mt-1">{{ periodLabelFull }}</p>
          </div>
        ...
        <div v-if="hasConversations" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <!-- CAC Card -->
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-border p-6 animate-fadeIn" style="animation-delay: 0.9s">
            ...
            <p class="text-xs text-text-secondary mt-1">{{ t.conversationsToOrders }}</p>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-border p-6 animate-fadeIn" style="animation-delay: 1s">
            ...
            <p class="text-xs font-medium text-text-secondary uppercase tracking-wider mb-1">{{ t.adSpend }}</p>
            <p class="text-2xl font-bold text-text-primary">${{ formatMoney(dashboardData?.financial?.metrics?.ad_spend || 0) }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div v-if="!hasMetrics" class="bg-white rounded-2xl shadow-sm border border-border p-6 animate-fadeIn" style="animation-delay: 1.1s">
...
const translations = {
  ...
  conversationsToOrders: { es: 'leads a órdenes', en: 'conversations to orders' },
