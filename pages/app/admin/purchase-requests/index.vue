<template>
    <section class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-primary-50/20">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              <h1 class="text-xl sm:text-2xl font-extrabold text-gray-900">{{ t.purchaseRequests }}</h1>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <!-- Filter Section -->
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6 p-4 animate-fadeIn">
          <div class="space-y-3">
              
            <!-- Bulk Actions Bar -->
            <div v-if="selectedRequests.length > 0" class="flex items-center justify-between p-3 bg-primary-50 border border-primary-200 rounded-xl">
              ...
              >
                  <svg v-if="!deletingBulk" class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
        ...
        <!-- Empty State -->
        <div v-else-if="requests.length === 0" class="text-center py-16 bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        <!-- List Table -->
        <div v-else class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fadeIn" style="animation-delay: 0.1s">
            ...
            <table class="w-full">
              <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t.request }}</th>
              ...
              <tbody class="divide-y divide-gray-100">
                ...
                >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="font-medium text-gray-900">{{ req.request_number }}</span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900 font-medium">{{ req.user?.name }}</div>
                    <div class="text-xs text-gray-500">{{ req.user?.email }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span :class="['px-2.5 py-0.5 rounded-full text-xs font-medium border', getStatusColor(req.status)]">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <svg class="w-5 h-5 text-gray-400 group-hover:text-primary-600 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full p-6">
                      <div class="sm:flex sm:items-start">
                          ...
                          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                              <h3 class="text-lg font-medium leading-6 text-gray-900">{{ t.confirmDelete }}</h3>
                              <div class="mt-2">
                                  <p class="text-sm text-gray-500">
                                      {{ t.confirmDeleteMessage.replace('{count}', selectedRequests.length) }}
                                  </p>
                              </div>
                          </div>
                      </div>
  ...
  const translations = {
    ...
    searchPlaceholder: { es: 'Buscar por #, cliente...', en: 'Search by #, customer...' },
    allStatuses: { es: 'Todos los estados', en: 'All Statuses' },
    loading: { es: 'Cargando...', en: 'Loading...' },
    noRequests: { es: 'No se encontraron solicitudes', en: 'No requests found' },
    noRequestsDesc: { es: 'Intenta con otros filtros.', en: 'Try different filters.' },
    ...
    confirmDeleteMessage: { es: '¿Estás seguro de eliminar {count} solicitud(es)? Esta acción no se puede deshacer.', en: 'Are you sure you want to delete {count} request(s)? This action cannot be undone.' },
