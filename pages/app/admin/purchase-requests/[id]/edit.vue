<template>
    <section class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-primary-50/20">
      ...
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <!-- Edit Form -->
        <form v-if="request" @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
              ...
              <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 animate-fadeIn">
                <h2 class="text-xl font-bold text-gray-900 mb-6">{{ t.generalInfo }}</h2>
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    <select
                      v-model="form.status"
                      class="w-full px-4 py-3 rounded-xl border border-gray-200 text-base focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all"
                    >
              ...
              <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 animate-fadeIn" style="animation-delay: 0.1s">
                <h2 class="text-xl font-bold text-gray-900 mb-6">{{ t.financials }} (USD)</h2>
                
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label class="block text-sm font-semibold text-gray-900 mb-2">{{ t.itemsTotal }}</label>
                    <input v-model.number="form.items_total" type="number" step="0.01" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500">
                  </div>
  
                  <div>
                    <label class="block text-sm font-semibold text-gray-900 mb-2">{{ t.shippingCost }}</label>
                    <input v-model.number="form.shipping_cost" type="number" step="0.01" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500">
                  </div>
  
                  <div>
                    <label class="block text-sm font-semibold text-gray-900 mb-2">{{ t.salesTax }}</label>
                    <input v-model.number="form.sales_tax" type="number" step="0.01" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500">
                  </div>
  
                  <div>
                    <label class="block text-sm font-semibold text-gray-900 mb-2">{{ t.fee }}</label>
                    <input v-model.number="form.processing_fee" type="number" step="0.01" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500">
                  </div>
  
                  <div class="sm:col-span-2">
                    <label class="block text-sm font-semibold text-gray-900 mb-2">{{ t.total }}</label>
                    <input v-model.number="form.total_amount" type="number" step="0.01" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500 font-bold bg-gray-50">
                  </div>
                </div>
              </div>
               ...
               <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 animate-fadeIn" style="animation-delay: 0.2s">
                <h2 class="text-xl font-bold text-gray-900 mb-6">{{ t.links }}</h2>
                <div>
                  <label class="block text-sm font-semibold text-gray-900 mb-2">{{ t.paymentLink }}</label>
                  <input v-model="form.payment_link" type="url" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500 text-sm text-blue-600">
                </div>
              </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3 sm:justify-between mt-8">
            ...
            <button
              ...
              class="order-1 sm:order-2 px-8 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white font-semibold rounded-xl shadow-lg hover:from-primary-600 hover:to-primary-700 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
  ...
  const translations = {
    ...
    notesPlaceholder: { es: 'Notas para el cliente o internas...', en: 'Notes for customer or internal...' },
    // Statuses
  ...
  const fetchRequest = async () => {
    loading.value = true
    try {
      const response = await $customFetch(`/admin/purchase-requests/${requestId}`)
